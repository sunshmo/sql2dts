...
var input = (0, import_path.join)(process.cwd(), args[0]);
var output = (0, import_path.join)(
  process.cwd(),
  args.includes("-o") ? args[args.indexOf("-o") + 1] : `${dbName}.d.ts`
);
if (!(0, import_node_fs.existsSync)(input)) {
  console.error("\u274C SQL file not found:", input);
  process.exit(1);
}
try {
  const { generate: generateDts } = globRequire_src(`../src/${dbName}`);
  const sql = (0, import_node_fs.readFileSync)(input, "utf-8");
  const dts = generateDts(sql);
  if (dts) {
    try {
      (0, import_node_fs.writeFileSync)(output, dts);
    } catch (err) {
      if (err.code === "ENOENT") {
        const dir = (0, import_path.dirname)(output);
